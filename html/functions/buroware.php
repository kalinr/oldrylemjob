<?
function exportBurowareOrders($date_start, $date_end, $date_end_adjusted, $email=0)
{
	$count =0;
	if(!$email)
	{
	...
	if($email) //export all non-exported
		...
		$query = "SELECT * FROM orders WHERE DATETIME >= '$date_start' AND DATETIME < '$date_end_adjusted'";
